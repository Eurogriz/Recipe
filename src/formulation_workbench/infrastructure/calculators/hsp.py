...
HSP_DATABASE: dict[str, HspPoint] = {
    ...
    "mineral spirits": HspPoint(
        "Mineral spirits", delta_d=16.0, delta_p=1.0, delta_h=2.0, radius=10.0
    ),
    ...
    "propylene glycol": HspPoint(
        "Propylene glycol", delta_d=16.8, delta_p=9.4, delta_h=23.3, radius=12.0
    ),
    "ethylene glycol": HspPoint(
        "Ethylene glycol", delta_d=17.0, delta_p=11.0, delta_h=26.0, radius=12.0
    ),
    # Polymers
    "acrylic (PMMA-like)": HspPoint(
        "Acrylic", delta_d=18.0, delta_p=10.0, delta_h=8.0, radius=10.0
    ),
...
class HspCalculator:
    ...
    def calculate_distance(cls, point_a: HspPoint, point_b: HspPoint) -> float:
        ...
        term_h = (_point_h := (point_a.delta_h - point_b.delta_h) ** 2)
