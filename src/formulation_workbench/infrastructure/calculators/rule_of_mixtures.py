...
class RuleOfMixturesCalculator:
    ...
    def calculate(
        ...
        voc_g_per_l = (
            voc_mass_per_100g / total_volume_per_100g if total_volume_per_100g > 0 else 0.0
        )
    ...
    @classmethod
    def _lookup_density(cls, name: str, densities: dict[str, float], notes: list[str]) -> float:
        ...
        if "pigment" in name_lower or any(
            p in name_lower for p in ("tio2", "titanium", "carbonate", "talc", "kaolin", "silica")
        ):
            return 2.500
