...
try:
    from sklearn.ensemble import GradientBoostingRegressor, RandomForestRegressor  # noqa: F401
...
class PropertyPredictor:
    ...
    def _extract_features(cls, recipe: Recipe) -> tuple[float, ...]:
        ...
        if "acrylic" in binder_lower:
            features[12] = 1.0
        if "polyurethane" in binder_lower or binder_lower.strip() == "pu":
            features[13] = 1.0
        ...
        for comp in recipe.all_components:
            ...
            # Categorize
            if any(
                kw in name_lower for kw in ("acrylic", "polyurethane", "alkyd", "epoxy", "binder")
            ):
                binder_mass += mass_pct
            elif (
                "titanium" in name_lower or "tio2" in name_lower or "13463-67-7" in comp.cas_number
            ):
                tio2_mass += mass_pct
                pigment_mass += mass_pct
            elif any(
                kw in name_lower for kw in ("carbonate", "talc", "kaolin", "silica", "filler")
            ):
    ...
    def train(
        ...
        for prop_name, y_values in training_data.items():
            ...
            if valid_mask.sum() < 30:
                logger.warning(
                    "Property %s: too few valid samples (%d)", prop_name, valid_mask.sum()
                )
                continue
            ...
            importance_dict = {
                name: float(imp)
                for name, imp in zip(self._feature_names, model.feature_importances_, strict=False)
            }
    ...
    @classmethod
    def _rule_based_fallback(cls, recipe: Recipe, property_name: str) -> PredictionResult | None:
        ...
        if property_name == "scrub_resistance_cycles":
            ...
            binder_pct = sum(
                ...
                if any(
                    kw in c.name.lower()
                    for kw in ("acrylic", "polyurethane", "alkyd", "epoxy", "binder")
                )
            )
