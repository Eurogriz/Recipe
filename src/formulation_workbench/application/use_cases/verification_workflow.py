...
class SubmitRecipeForReviewUseCase:
    ...
    async def execute(self, command: SubmitRecipeForReviewCommand) -> Recipe:
        ...
        _is_valid, violations = VerificationRules.can_be_verified(existing)
    ...
    async def execute(self, command: VerifyRecipeCommand) -> Recipe:
        ...
        if new_recipe.status.is_verified:
            try:
                ...
                logger.error("Cannot transition to VERIFIED due to rule violations: %s", e)
                raise ValueError(f"Verification rules not satisfied: {e}") from e
