...
class CompareRecipesUseCase:
    ...
    async def execute(self, query: CompareRecipesQuery) -> ComparisonResult:
        ...
        if len(query.recipe_ids) < 2:
            raise ValueError(f"Comparison requires at least 2 recipes, got {len(query.recipe_ids)}")
        if len(query.recipe_ids) > self.MAX_RECIPES:
            raise ValueError(
                f"Cannot compare more than {self.MAX_RECIPES} recipes, got {len(query.recipe_ids)}"
            )
    ...
    def _build_composition_rows(self, recipes: tuple[Recipe, ...]) -> tuple[ComparisonRow, ...]:
        ...
        for comp_name in sorted(all_components):
            ...
            for recipe in recipes:
                ...
                if comps:
                    values.append(f"{comps[0].mass_percent:.2f}% (CAS: {comps[0].cas_number})")
                else:
                    values.append("—")
    ...
    def _build_properties_rows(self, recipes: tuple[Recipe, ...]) -> tuple[ComparisonRow, ...]:
        ...
        property_fields = [
            ...
            (
                "Verification count",
                lambda r: f"{r.status.verification_count}/{r.status.required_verifications}",
            ),
