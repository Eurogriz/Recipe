...
class BatchCalculator:
    ...
    def calculate_for_mass(
        ...
        if abs(total_percent - 100.0) > 1.0:
            warnings.append(
                f"⚠ Mass percents sum to {total_percent:.2f}% (expected 100%)"
            )

        # Compute scale factor
        scale_factor = target_mass_kg / (total_percent / 100.0)
        ...
        for comp in recipe.all_components:
            ...
            if comp.name.lower() not in densities:
                warnings.append(
                    f"⚠ No density for '{comp.name}', assuming 1.0 g/cm³"
                )
    ...
    @classmethod
    def _estimate_average_density(
        cls, recipe: Recipe, densities: dict[str, float]
    ) -> float:
