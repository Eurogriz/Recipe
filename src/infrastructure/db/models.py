...
class UserModel(Base):
    ...
    role: Mapped[str] = mapped_column(String(32), nullable=False)  # Viewer/Technologist/Admin/Auditor
...
class RawMaterialModel(Base):
    ...
    inci_name: Mapped[Optional[str]] = mapped_column(String(256))
    function: Mapped[str] = mapped_column(String(128), nullable=False)
    density_g_per_cm3: Mapped[Optional[float]] = mapped_column(Float)
    molar_mass: Mapped[Optional[float]] = mapped_column(Float)
...
class RecipeModel(Base):
    ...
    finish: Mapped[Optional[str]] = mapped_column(String(64))
    ...
    verified_by: Mapped[Optional[str]] = mapped_column(String(36), ForeignKey("user.id"))
    verified_at: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True))

    version: Mapped[int] = mapped_column(Integer, nullable=False, default=1)
    previous_version_id: Mapped[Optional[str]] = mapped_column(
        String(36), ForeignKey("recipe.id")
    )
...
class CompositionStageModel(Base):
    ...
    equipment: Mapped[Optional[str]] = mapped_column(String(256))
    rotational_speed_rpm: Mapped[Optional[float]] = mapped_column(Float)
    peripheral_speed_m_per_s: Mapped[Optional[float]] = mapped_column(Float)
    temperature_c: Mapped[Optional[float]] = mapped_column(Float)
    duration_min: Mapped[Optional[int]] = mapped_column(Integer)
    control_points_json: Mapped[Optional[str]] = mapped_column(Text)
...
class ComponentModel(Base):
    ...
    stage_id: Mapped[str] = mapped_column(
        String(36), ForeignKey("composition_stage.id", ondelete="CASCADE"), nullable=False, index=True
    )
    ...
    inci_name: Mapped[Optional[str]] = mapped_column(String(256))
    function: Mapped[str] = mapped_column(String(128), nullable=False)
    manufacturer_reference: Mapped[Optional[str]] = mapped_column(Text)
    ...
    stage: Mapped[CompositionStageModel] = relationship("CompositionStageModel", back_populates="components")

    __table_args__ = (
        CheckConstraint("mass_percent >= 0 AND mass_percent <= 100", name="ck_mass_percent_range"),
        CheckConstraint("tolerance_percent >= 0 AND tolerance_percent <= 100", name="ck_tolerance_range"),
    )
...
class RecipeSourceReferenceModel(Base):
    ...
    page_or_formula: Mapped[Optional[str]] = mapped_column(String(256))
