...
class SqlAlchemyRecipeRepository:
    ...
    async def get_by_id(self, recipe_id: str) -> Recipe | None:
        model = await self._session.get(
            RecipeModel, recipe_id, options=[selectinload(RecipeModel.stages).selectinload(CompositionStageModel.components)]
        )
        if model is None:
            return None
    ...
    async def count_by_status(self) -> dict[VerificationState, int]:
        ...
        result = await self._session.execute(stmt)
        rows = result.all()
        counts: dict[VerificationState, int] = {state: 0 for state in VerificationState}
    ...
    def _to_model(self, recipe: Recipe) -> RecipeModel:
        ...
        for stage in recipe.stages:
            stage_model = CompositionStageModel(
                ...
                peripheral_speed_m_per_s=stage.process.peripheral_speed_m_per_s if stage.process else None,
                ...
                control_points_json=json.dumps(list(stage.process.control_points)) if stage.process else None,
            )
